...
// draw from public/controller/ folder
...
window.TouchControls = class TouchControls {
	constructor(mobile = true) {
		...
		this.buttons = {};
		this.touchState = this.touchState || new Map();
		...
		this.gamepadConfig = {
			specialButtons: {
				...
				buttons: [
					{ id: "pause", border: false, label: "Pause", color1: "rgba(255, 255, 255, 0)", color2: "rgba(255, 255, 0, 0)", active: "pause-active.png", inactive: "pause-inactive.png", x: "36px", y: "0px", width: "30px", height: "30px", key: "Escape", keyCode: 27 },
					...
					{ id: "item", border: false, label: "Item", color1: "rgba(255, 255, 255, 0)", color2: "rgba(255, 255, 0, 0)", x: "calc(50% - 30px)", active: "item-active.png", inactive: "item-inactive.png", y: "0px", width: "60px", height: "30px", key: "q", keyCode: 81 },
					{ id: "toggleVisibility", border: false, label: "Hide Buttons", color1: "rgba(255, 255, 255, 0)", color2: "rgba(255, 255, 0, 0)", x: "calc(100% - 66px)", active: "visibility-active.png", inactive: "visibility-inactive.png", y: "0px", width: "30px", height: "30px", key: "", keyCode: null },
			...
			dpad: {
				...
				buttons: [
					{ id: "up", label: "↑", border: false, color1: "rgba(255, 255, 255, 0)", color2: "rgba(255, 255, 0, 0)", active: "up-active.png", inactive: "up-inactive.png", x: 50, y: 0, width: 50, height: 50, key: "w", keyCode: 87 },
					{ id: "left", label: "←", border: false, color1: "rgba(255, 255, 255, 0)", color2: "rgba(255, 255, 0, 0)", active: "left-active.png", inactive: "left-inactive.png", x: 0, y: 50, width: 50, height: 50, key: "a", keyCode: 65 },
					{ id: "right", label: "→", border: false, color1: "rgba(255, 255, 255, 0)", color2: "rgba(255, 255, 0, 0)", active: "right-active.png", inactive: "right-inactive.png", x: 100, y: 50, width: 50, height: 50, key: "d", keyCode: 68 },
					{ id: "down", label: "↓", border: false, color1: "rgba(255, 255, 255, 0)", color2: "rgba(255, 255, 0, 0)", active: "down-active.png", inactive: "down-inactive.png", x: 50, y: 100, width: 50, height: 50, key: "s", keyCode: 83 },
					...
					{ id: "up-left", label: "↖", border: false, color1: "rgba(255, 255, 255, 0)", color2: "rgba(255, 255, 0, 0)", active: "", inactive: "", x: 0, y: 0, width: 50, height: 50, keys: ["w","a"], keyCodes: [87,65] },
					{ id: "up-right", label: "↗", border: false, color1: "rgba(255, 255, 255, 0)", color2: "rgba(255, 255, 0, 0)", active: "", inactive: "", x: 100, y: 0, width: 50, height: 50, keys: ["w","d"], keyCodes: [87,68] },
					{ id: "down-left", label: "↙", border: false, color1: "rgba(255, 255, 255, 0)", color2: "rgba(255, 255, 0, 0)", active: "", inactive: "", x: 0, y: 100, width: 50, height: 50, keys: ["s","a"], keyCodes: [83,65] },
					{ id: "down-right", label: "↘", border: false, color1: "rgba(255, 255, 255, 0)", color2: "rgba(255, 255, 0, 0)", active: "", inactive: "", x: 100, y: 100, width: 50, height: 50, keys: ["s","d"], keyCodes: [83,68] }
				]
			},
			actionButtons: {
	...
	simulateKeyEvent(keyInfo, type) {
		...
		const iframeDocument = null //iframe.contentWindow.document;

		const event = new KeyboardEvent(type, {
			bubbles: true,
			cancelable: true,
			key: keyInfo.key,
			code: `${keyInfo.key == "Escape" ? keyInfo.key : "Key" + keyInfo.key.toUpperCase()}`,
...
//iframeDocument.dispatchEvent(event);
	...
	/**
	 * Helper: given a touch point, determine which dpad directions
	 * should be active based on its position relative to the dpad bounds.
	 * Returns a Set(["up","right"]) etc.
	 */
	getDpadDirectionsFromPoint(clientX, clientY) {
		const dpad = document.getElementById("dpad");
		if (!dpad) return new Set();
		...
		// Dead-zone tolerance so tiny jitters don't flicker
		const threshold = Math.min(rect.width, rect.height) * 0.12;

		if (dy < -threshold) dirs.add("up");
		if (dy > threshold)  dirs.add("down");
		if (dx < -threshold) dirs.add("left");
		if (dx > threshold)  dirs.add("right");
	...
	handleButtonPress(buttonId) {
		...
		if (buttonId === "fullscreen") {
			...
			if (document.body.webkitRequestFullscreen) return document.body.webkitRequestFullscreen();
		...
		if (buttonId === "toggleVisibility") {
			currentButton.element.style.backgroundImage = `url("${window.assetsURL}/${this.gamepadVisible ? currentButton.active : currentButton.inactive}")`;
			document.querySelectorAll("#touch-gamepad button").forEach(btn => {
				if (["settings", "toggleVisibility"].includes(btn.id)) return;
			...
			document.querySelectorAll("#touch-gamepad div").forEach(container => {
				if (container.id == "special") return;
				if (this.gamepadVisible) {
	...
	handleTouchStart(event) {
		...
		Array.from(event.changedTouches).forEach(touch => {
			...
			if (!(buttonId in this.buttons)) return;
	
			 // Determine zone
			const inDpad = document.getElementById("dpad")?.contains(element);
			const inAction = document.getElementById("action-buttons")?.contains(element);
	
			const zone = inDpad ? "dpad" : (inAction ? "action" : "other");
	
			// Track state for this touch
			const state = {
				origin: buttonId,
				active: new Set([buttonId]),
				zone
			};
	
			this.touchState.set(touch.identifier, state);
			this.touchMap.set(touch.identifier, buttonId);
			this.handleButtonPress(buttonId);
	
			// For D-Pad, immediately evaluate multi-direction
			if (zone === "dpad") {
				const dirs = this.getDpadDirectionsFromPoint(touch.clientX, touch.clientY);
				dirs.forEach(dir => {
					if (!state.active.has(dir) && dir in this.buttons) {
						state.active.add(dir);
	...
	handleTouchMove(event) {
		...
		Array.from(event.changedTouches).forEach(touch => {
			...
			// --- D-PAD LOGIC ---
			if (state.zone === "dpad") {
				const desired = this.getDpadDirectionsFromPoint(touch.clientX, touch.clientY);
	
				// Release anything no longer desired (including the origin if crossed)
				Array.from(state.active).forEach(dir => {
					if (!desired.has(dir)) {
						state.active.delete(dir);
						this.handleButtonRelease(dir);
					}
				});
	
				// Press newly-desired directions
				desired.forEach(dir => {
					if (!state.active.has(dir) && dir in this.buttons) {
						state.active.add(dir);
						this.handleButtonPress(dir);
			...
			// --- ACTION BUTTON LOGIC ---
			if (state.zone === "action") {
				const element = document.elementFromPoint(touch.clientX, touch.clientY);
				const overId = element?.id;
	
				// Origin button stays latched until touch end
				// Other buttons only active while touch is on them
				if (overId && overId in this.buttons) {
					if (!state.active.has(overId)) {
						state.active.add(overId);
						this.handleButtonPress(overId);
					}
				}
	
				// Release any non-origin buttons we slid off of
				Array.from(state.active).forEach(id => {
					if (id !== state.origin && id !== overId) {
			...
			// --- DEFAULT (unchanged old behavior) --- 
			const element = document.elementFromPoint(touch.clientX, touch.clientY);
			const newButton = element?.id;
			const oldButton = this.touchMap.get(touch.identifier);
	
			 if (oldButton !== newButton) {
				if (oldButton in this.buttons) this.handleButtonRelease(oldButton);
	...
	handleTouchEnd(event) {
		...
		Array.from(event.changedTouches).forEach(touch => {
			...
			// Release everything that touch was responsible for
			state.active.forEach(id => this.handleButtonRelease(id));
...
window.SettingsMenu = class SettingsMenu {
	...
	enableLayoutEditing(enable) {
		this.layoutEditing = enable;
	
		const movable = ["dpad","action-buttons"];
		movable.forEach(id => {
			const el = document.getElementById(id);
			if (!el) return;
			...
			if (enable) {
				...
				const startDrag = e => {
					...
					// Measure offset from bottom edge
					this.dragOffset.y = window.innerHeight - y - parseFloat(el.style.bottom || 0);
				...
				const moveDrag = e => {
					...
					this.dragTarget.style.left = (x - this.dragOffset.x) + "px";
				
					const bottomVal = (window.innerHeight - y - this.dragOffset.y);
					this.dragTarget.style.bottom = bottomVal + "px";
				};
				...
				el._layoutHandlers = { startDrag, moveDrag, endDrag };
				...
				el.style.cursor = "";
				if (!el._layoutHandlers) return;
	
				const { startDrag, moveDrag, endDrag } = el._layoutHandlers;
	...
	saveSettings() {
		...
		["dpad","action-buttons"].forEach(id => {
			const el = document.getElementById(id);
			if (!el) return;
			this.settings.positions[id] = {
				left: getComputedStyle(el).left || "0px",
				bottom: getComputedStyle(el).bottom || "0px"
			};
		});

		localStorage.setItem("winrarisyou.TouchControlsSettings", JSON.stringify({
			"controlScale": this.settings.controlScale, 
	...
	resetSettings() {
		localStorage.setItem("winrarisyou.TouchControlsSettings", JSON.stringify({
			"controlScale": "1", 
			"controlOpacity": "1",
			"positions": { "dpad": { "left": "10px", "bottom": "60px" }, "action-buttons": { "left": "535px", "bottom": "60px" } }
		}));

		this.settings = JSON.parse(localStorage.getItem("winrarisyou.TouchControlsSettings"))

		document.querySelectorAll("#touch-gamepad button").forEach(btn => {
			if (!["settings", "toggleVisibility"].includes(btn.id)) {
				btn.style.opacity = this.settings.controlOpacity;
			}
		});
		this.opacityLabel.innerText = `Controls Opacity: ${Math.round(this.settings.controlOpacity ?? "1") * 100}%`;
		this.opacitySlider.value = this.settings.controlOpacity;

		this.containerIds.forEach(id => {
			if (window.debugmode) window.touchControlsInstance.updateDebugInfo();
			this.setScale(id, this.settings.controlScale);
		});
		this.scaleLabel.innerText = `Button Scale: ${Math.round((this.settings.controlScale ?? "1") * 100)}%`;
		this.scaleSlider.value = this.settings.controlScale;
		this.applySavedPositions()
	}

	createLayoutResetButton() {
		const layoutBtn = document.createElement("button");
		layoutBtn.innerText = "Reset layout to default";
		layoutBtn.style.marginTop = "5px";
		layoutBtn.style.marginBottom = "5px";
		layoutBtn.style.display = "inline-block";

		layoutBtn.addEventListener("click", () => {
			this.resetSettings();
		});
		this.settingsModalElem.appendChild(layoutBtn);
		this.settingsModalElem.insertAdjacentHTML("beforeend", `<br>`)
	}

	createLayoutButton() {
		const layoutBtn = document.createElement("button");
		layoutBtn.innerText = this.layoutEditing ? "Stop Editing Layout" : "Edit Layout";
		layoutBtn.style.marginTop = "5px";
		layoutBtn.style.marginBottom = "5px";
		layoutBtn.style.display = "inline-block";

		layoutBtn.addEventListener("click", () => {
			this.enableLayoutEditing(!this.layoutEditing);
			layoutBtn.innerText = this.layoutEditing ? "Stop Editing Layout" : "Edit Layout";
			this.close(window.settingsButton)
		});
		this.settingsModalElem.appendChild(layoutBtn);
		this.settingsModalElem.insertAdjacentHTML("beforeend", `<br>`)
	}
	...
	createCloseButton() {
		if (!document.getElementById("closeSettingsMenu")) {
			...
			this.closeButton.style.fontFamily = "Tomorrow-Medium";
	...
	createOpacityControls() {
		...
		if (!document.getElementById("opacityLabel")) {
			...
			this.opacityLabel.style.paddingTop = "5px";
			this.opacityLabel.style.paddingBottom = "5px";
			this.opacityLabel.style.display = "inline-block";
			this.opacityLabel.innerText = `Controls Opacity: ${(this.settings.controlOpacity ?? "1") * 100}%`;
		...
		if (!document.getElementById("opacitySlider")) {
			this.opacitySlider = this.createSlider(0, 1, 0.01, (this.settings.controlOpacity ?? "1"));
			this.opacitySlider.id = "opacitySlider"
			this.settingsModalElem.appendChild(this.opacitySlider);

			this.opacitySlider.addEventListener("input", () => {
				this.settings.controlOpacity = this.opacitySlider.value;
	...
	createScaleControls() {
		...
		if (!document.getElementById("scaleLabel")) {
			...
			this.scaleLabel.style.paddingTop = "5px";
			this.scaleLabel.style.paddingBottom = "5px";
			this.scaleLabel.style.display = "inline-block";
			this.scaleLabel.innerText = `Button Scale: ${Math.round((this.settings.controlScale ?? "1") * 100)}%`;
		...
		if (!document.getElementById("scaleSlider")) {
			this.scaleSlider = this.createSlider(0.5, 2, 0.01, (this.settings.controlScale ?? "1"));
			...
			this.scaleSlider.addEventListener("input", () => {
				...
				this.settings.controlScale = this.scaleSlider.value;
				this.scaleLabel.innerText = `Button Scale: ${Math.round(100 * this.settings.controlScale)}%`;
		...
		this.containerIds.forEach(id => {
			if (window.debugmode) window.touchControlsInstance.updateDebugInfo();
			this.setScale(id, (this.settings.controlScale ?? "1"));
		});
	}
	...
	createVersionLabel() {
		if (!document.getElementById("versionLabel")) {
			...
			this.versionLabel.style.paddingBottom = "5px";
			this.versionLabel.style.display = "inline-block";
